...
.DESCRIPTION
    ...
    Layout parameters can be provided by the site or page, or command line.
#>
param(        
    # If this is not provided, it should default to a value from `$site` or `$page
    [Alias('PageTitle')]
    [string]
    $Title = $(
    ...
    # One or more objects to put into the page body.
    [Alias('PageBody')]
    [PSObject[]]
    $Body,
    ...
    # The font name.
    # This should be used throughout the page or site.
    # It should be presumed to be a Google Font.
    [Alias('FontName')]
    [string]
    $Font = $(
    ...
    # The code font name.
    # This should be used throughout code in the page or site.
    # It should be presumed to be a Google Font.
    [string]
    $CodeFont = $(
...
filter outputHtml {
    ...
    switch ($outputItem) {
        ...
        {$outputItem -is [IO.FileInfo]} {
            $outputItemRelativeSafePath =                 
                $outputItem.FullName -replace "^$([Regex]::Escape(
                    $(if ($RootPath) {
                        $RootPath
                    } else {
                        $pwd
            ...
            $outputLinkContent = switch ($outputItem.Extension) {
                .svg {                    
                    Get-Content -LiteralPath $outputItem.FullName -Raw                    
                }
                { $_ -in '.png', '.jpg', '.jpeg', '.gif'} {
                    "<img src='$outputItemRelativeSafePath' alt='$(
                        $outputItem.Name | htmlAttributeEncode
                    )'></img>"
                }
                default {            
                    $outputItem.Name | htmlEncode
                }
            }

            "<a href='$outputItemRelativeSafePath'>$outputLinkContent</a>"
        }
...
$(
    if ($HeaderMenu) {
        ...
        "@media (orientation: portrait) {"
            ".header-menu { display: grid; grid-template-columns: repeat(auto-fit, minmax(66px, 1fr)); gap: 0.25em; position: sticky }"
...
$headerElements = @(
    ...
    # * Color palette
    if ($PaletteName) {
        $potentialPalettePath  =
            if ($site.Path) { Join-Path $site.Path "palettes/$($PaletteName)/$($paletteName).css" }
        if ($site.Path -and $potentialPalettePath -and (Test-Path -ErrorAction Ignore $potentialPalettePath)) {
            "<link rel='stylesheet' href='/palettes/$($PaletteName)/$($paletteName).css' id='palette' />"
        } else {
    ...
    if ($ImportMap -is [Collections.IDictionary]) {
        if (-not $ImportMap.imports) {
            "<script type='importmap'>$(@{imports = $ImportMap} | ConvertTo-Json -Depth 4)</script>"    
        } else {
            "<script type='importmap'>$($ImportMap | ConvertTo-Json -Depth 4)</script>"
        }        
    }
